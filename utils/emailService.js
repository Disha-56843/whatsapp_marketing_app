// emailService.js - Using Mailtrap Email API (HTTP-based, works on Render)
// No SMTP, no domain needed, free up to 1000 emails/month
...
export const sendOTPEmail = async (email, otp, type = "verify") => {
  try {
    ...
    const response = await fetch("https://send.api.mailtrap.io/api/send", {
      ...
      body: JSON.stringify({
        from: {
          email: "hello@demomailtrap.co",  // Mailtrap's free demo sender
    ...
    if (!response.ok) {
      ...
      throw new Error(data?.errors?.join(", ") || "Failed to send email");
    }
    ...
    throw new Error("Failed to send OTP email. Check your MAILTRAP_API_TOKEN.");
  }
};