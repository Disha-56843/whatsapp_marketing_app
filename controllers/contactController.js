...
export const getContacts = async (req, res) => {
  try {
    const contacts = await Contact.find({ owner: req.userId }).sort({ createdAt: -1 });
    return successResponse(res, { contacts, count: contacts.length });
  } catch (error) {
...
export const addContact = async (req, res) => {
  ...
  try {
    ...
    const exists = await Contact.findOne({ phone, owner: req.userId });
    if (exists) return errorResponse(res, "A contact with this phone number already exists.", 409);
    ...
    return errorResponse(res, "Failed to add contact.");
  }
};
...
export const deleteContact = async (req, res) => {
  try {
    ...
    return errorResponse(res, "Failed to delete contact.");
  }
};
...
export const importContacts = async (req, res) => {
  try {
    ...
    if (!Array.isArray(contacts) || contacts.length === 0) {
      return errorResponse(res, "No contacts provided.", 400);
    }
    ...
    if (validContacts.length === 0) {
      return errorResponse(res, "No valid contacts found. Phone number is required.", 400);
    }
    ...
    return successResponse(
      res,
      { count: result.length, batch, skipped: contacts.length - result.length },
      `${result.length} contacts imported successfully.`,
      201
    );
  } catch (error) {
    // Handle duplicate key errors gracefully
    if (error.code === 11000 && error.insertedDocs) {
      return successResponse(
        ...
        "Contacts imported. Duplicates were skipped."
      );
    }
    console.error("❌ Import contacts error:", error);
    return errorResponse(res, "Failed to import contacts.");
  }
};